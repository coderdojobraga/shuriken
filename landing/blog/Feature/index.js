...
export default function Feature({ title, author, date, photo, topic}) {
    return (
        <div class="mt-4 flex flex-col max-w-sm px-8 py-6 bg-white rounded-lg shadow-md">
            <div class="flex items-center"><a href="#"
                class="px-2 py-1 text-sm text-white bg-primary rounded">{topic}</a>
            </div>
            <div class="mt-4">
                <a href="#" class="text-lg font-bold text-dark hover:underline">
                    {title}
                </a>
            </div>
