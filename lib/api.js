...
export async function getAvailabilities(event_id) {
  const response = await API.get(`/api/events/${event_id}/availabilities`);

  return response.data;
}

export async function getAvailableMentors(event_id) {
  const response = await API.get(`/api/events/${event_id}/availabilities`);

  return response.data.data.filter((mentor) => mentor.is_available == true);
}

export async function getEnrolledNinjas(event_id, guardian_id) {
  const response = await API.get(`/api/events/${event_id}/enrollments`);

  return response.data.data.filter(
    (entity) => entity?.ninja.guardian_id == guardian_id
  );
}

export async function createEnrollment(ninja_id, event_id, accepted) {
